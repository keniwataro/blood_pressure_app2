...
患者・医療従事者・システム管理者による役割分担型のシステムをサポートしており、Rails 7.1.5.2で構築されています。
最新スキーマバージョン: 2025_10_09_091401
...
| `current_hospital_role_id` | bigint | YES | - | 現在の病院役割ID | FOREIGN KEY (user_hospital_roles.id) |

**外部キー制約:**
- `current_hospital_role_id` → `user_hospital_roles.id`
...
- `belongs_to :current_hospital_role, class_name: 'UserHospitalRole'` (N:1)
- `has_many :patient_staff_assignments_as_patient` (患者としての割り当て)
- `has_many :patient_staff_assignments_as_staff` (スタッフとしての割り当て)
...
| `created_at` | timestamp | NO | - | 作成日時 | - |
| `updated_at` | timestamp | NO | - | 更新日時 | - |
| `permission_level` | integer | NO | 0 | 権限レベル (0: general, 1: administrator) | INDEX |
...
- `users.current_hospital_role_id`
- `roles.name` (unique)
...
- `patient_staff_assignments.hospital_id, patient_id, staff_id` (unique)
...
6. 現在の役割参照変更 (current_role_id → current_hospital_role_id)
7. 現在の病院役割IDのNULL制約解除
